...
public class WarrantyController {
    ...
    @FXML private TableColumn<Warranty, Integer> recordColumn;
    ...
    private void showWarranty(Warranty warranty) { if (warranty == null) return; recordIdField.setText(String.valueOf(warranty.getServiceRecordId())); warrantyCodeField.setText(warranty.getWarrantyCode()); startDatePicker.setValue(warranty.getStartDate()); endDatePicker.setValue(warranty.getEndDate()); statusComboBox.setValue(warranty.getStatus()); coverageArea.setText(warranty.getCoverage()); }
    private int recordId() { int id = Integer.parseInt(recordIdField.getText().trim()); if (id <= 0) throw new IllegalArgumentException("Service Record ID phải lớn hơn 0."); return id; }
    private String warrantyCode() { String code = warrantyCodeField.getText().trim(); if (code.isEmpty()) throw new IllegalArgumentException("Mã bảo hành là bắt buộc."); return code; }
    private void validate() { recordId(); warrantyCode(); if (startDatePicker.getValue() == null || endDatePicker.getValue() == null) throw new IllegalArgumentException("Ngày bắt đầu và kết thúc là bắt buộc."); if (endDatePicker.getValue().isBefore(startDatePicker.getValue())) throw new IllegalArgumentException("Ngày kết thúc phải sau ngày bắt đầu."); if (statusComboBox.getValue() == null) throw new IllegalArgumentException("Hãy chọn trạng thái."); }
